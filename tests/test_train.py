...
@pytest.mark.asyncio
async def test_trainer_initialization(mock_graph, mock_dataset, mock_optimizer, mock_module_class):
...
@pytest.mark.asyncio
async def test_trainer_step(mock_graph, mock_dataset, mock_optimizer, mock_module_class):
...
def test_trainer_train_with_failed_cases(mock_graph, mock_dataset, mock_optimizer, mock_module_class):
...
def test_trainer_early_stopping(mock_graph, mock_dataset, mock_optimizer, mock_module_class):
...
def test_trainer_train_with_no_losses(mock_graph, mock_dataset, mock_optimizer, mock_module_class):
    ...
    assert mock_optimizer.step.call_count == 2
    assert mock_optimizer.zero_grad.call_count == 2
    assert mock_case.output.text_grad == ""  # No backward pass since all assertions passed 